...
public class LeaveController {
	...
	public ResponseEntity<ResponseMessage<LeaveApproveResponseDTO>> approveLeave(@PathVariable Long id,
			@Valid @RequestBody LeaveApproveRequestDTO leave) {
		  LeaveApproveResponseDTO updatedLeave = leaveServiceImpl.approveLeave(id, leave);
	        
	        if (updatedLeave != null) {
	            ResponseMessage<LeaveApproveResponseDTO> response = new ResponseMessage<>(true, 1,
	                    "Status updated successfully", updatedLeave);
	            return ResponseEntity.status(HttpStatus.OK).body(response);
	        } else {
	            ResponseMessage<LeaveApproveResponseDTO> response = new ResponseMessage<>(false, 0,
